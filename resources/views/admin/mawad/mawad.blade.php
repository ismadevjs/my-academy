...
<div class="mb-4">
    <div class="row">
      <div class=" mb-4 col-md-6 col-xl-4">
        <a data-bs-toggle="modal" data-bs-target="#add-type" class="block block-rounded block-transparent block-link-pop bg-gd-dusk h-100 mb-0" href="javascript:void(0)">
    ...
    <div class="modal-dialog modal-dialog-slideleft" role="document">
      <div class="modal-content">
        <div class="block block-rounded block-themed block-transparent mb-0">
          ...
          <form action="{{route('admin.mawads.add')}}" method="POST" enctype="multipart/form-data">
      ...
      <table class="js-table-checkable table table-bordered table-striped table-vcenter">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="d-none d-sm-table-cell">
                <span class="">{{ $mawad->name }}</span>
              </td>
              <td class="d-none d-sm-table-cell">
                <span class="">{{ $mawad->sanawatss->name }}</span>
              </td>
            <td class="d-none d-sm-table-cell">
              <em class="fs-sm text-muted">{{ $mawad->created_at->diffForHumans() }}</em>
            </td>
            <td class="d-none d-sm-table-cell">
                <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#e-{{$mawad->id}}">E</button>
                <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#d-{{$mawad->id}}">D</button>
            </td>
          </tr>
          
  
           <!-- Slide Left Block Modal chack suppressions -->
           <div class="modal fade" id="e-{{$mawad->id}}" tabindex="-1" role="dialog" aria-labelledby="modal-block-slideleft" aria-hidden="true">
            <div class="modal-dialog modal-dialog-slideleft" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-themed block-transparent mb-0">
                <div class="block-header bg-primary-dark">
                ...
                <div class="block-content text-center ">
                    <h4 class=" text-bold ">mawad : <span class="badge bg-danger">{{$mawad->name}}</span> Will be edited</h4>
                </div>
                <form action="{{route('admin.mawads.update')}}" method="POST" enctype="multipart/form-data">
                    <div class="block-content">
                      @csrf
                      ...
                      <div class="mb-4">
                          <label for="">Name : </label>
                          <input type="hidden" name="id" value="{{$mawad->id}}" class="form-control">
                          <input type="text" name="name" value="{{$mawad->name}}" class="form-control">
                      </div>
  ...
  <!-- Slide Left Block Modal chack suppressions -->
    <div class="modal fade" id="d-{{$mawad->id}}" tabindex="-1" role="dialog" aria-labelledby="modal-block-slideleft" aria-hidden="true">
      <div class="modal-dialog modal-dialog-slideleft" role="document">
        <div class="modal-content">
          <div class="block block-rounded block-themed block-transparent mb-0">
            <div class="block-header bg-primary-dark">
            ...
            <div class="block-content text-center ">
              <h4 class=" text-bold ">mawad : <span class="badge bg-danger">{{$mawad->name}}</span> Will be deleted</h4>
            </div>
