...
<div class="mb-4">
  ...
  <div class="block-header block-header-default">
    <h3 class="block-title">List taalims</h3>
      ...
      <div class="modal-content">
        <div class="block block-rounded block-themed block-transparent mb-0">
          <div class="block-header bg-primary-dark">
            <h3 class="block-title">Ajouter taalim</h3>
            <div class="block-options">
          ...
          <form action="{{route('admin.taalim.add')}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="modal-dialog modal-dialog-slideleft" role="document">
      <div class="modal-content">
        <div class="block block-rounded block-themed block-transparent mb-0">
            ...
            <form action="{{route('admin.taalim.delete.all')}}" method="POST">
              @csrf
            <div class="block-content block-content-full text-end bg-body">
              <button type="button" class="btn btn-sm btn-alt-secondary" data-bs-dismiss="modal">Close</button>
      ...
      <table class="js-table-checkable table table-bordered table-striped table-vcenter">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="d-none d-sm-table-cell">
                <span class="">{{ $taalim->name }}</span>
              </td>
            
            <td class="d-none d-sm-table-cell">
              <em class="fs-sm text-muted">{{ $taalim->created_at->diffForHumans() }}</em>
            </td>
            <td class="d-none d-sm-table-cell">
                <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#e-{{$taalim->id}}">E</button>
                <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#d-{{$taalim->id}}">D</button>
            </td>
          </tr>
          
  
           <!-- Slide Left Block Modal chack suppressions -->
           <div class="modal fade" id="e-{{$taalim->id}}" tabindex="-1" role="dialog" aria-labelledby="modal-block-slideleft" aria-hidden="true">
            <div class="modal-dialog modal-dialog-slideleft" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-themed block-transparent mb-0">
                <div class="block-header bg-primary-dark">
                ...
                <div class="block-content text-center ">
                    <h4 class=" text-bold ">Taalim : <span class="badge bg-danger">{{$taalim->name}}</span> Will be edited</h4>
                </div>
                <form action="{{route('admin.taalim.update')}}" method="POST" enctype="multipart/form-data">
                    <div class="block-content">
                      @csrf
                      <div class="mb-4">
                          <label for="">Name : </label>
                          <input type="hidden" name="id" value="{{$taalim->id}}" class="form-control">
                          <input type="text" name="name" value="{{$taalim->name}}" class="form-control">
                      </div>
  ...
  <!-- Slide Left Block Modal chack suppressions -->
    <div class="modal fade" id="d-{{$taalim->id}}" tabindex="-1" role="dialog" aria-labelledby="modal-block-slideleft" aria-hidden="true">
      <div class="modal-dialog modal-dialog-slideleft" role="document">
        <div class="modal-content">
          <div class="block block-rounded block-themed block-transparent mb-0">
            <div class="block-header bg-primary-dark">
            ...
            <div class="block-content text-center ">
              <h4 class=" text-bold ">Taalim : <span class="badge bg-danger">{{$taalim->name}}</span> Will be deleted</h4>
            </div>
